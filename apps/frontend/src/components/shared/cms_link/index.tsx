...
export function urlToRelative(url?: URL | string | undefined): string {
  ...
  const linkUrl =
    typeof url == "string"
      ? new URL(url, `https:/${process.env.NEXT_PUBLIC_SITE_DOMAIN}/`)
      : url;
  const relativePath = isPrimaryHost(linkUrl.host)
...
function isPrimaryHost(host: string): boolean {
  try {
    const isPrimary =
      host == process.env.SITE_PRIMARY ||
      host == process.env.NEXT_PUBLIC_SITE_PRIMARY ||
